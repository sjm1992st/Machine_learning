...
#################
def sort_data(m_data_x):
    [xx,yy]=m_data_x.shape
    for i in range(xx-1):
        for j in range(i+1,xx):
            if m_data_x[i,0]>m_data_x[j,0]:
                temp=copy.deepcopy(m_data_x[i,:])
                m_data_x[i,:]=m_data_x[j,:]
                m_data_x[j,:]=copy.deepcopy(temp)
    return m_data_x
...
plt.plot(X, y_kr, c='r', label='Kernel Regression') ##line -plt.plot(X, y_svr, c='r', label='SVR'),plt.scatter(X, y_kr, c='b', label='Kernel Regression')
